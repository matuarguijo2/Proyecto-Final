...
import { getMisCampanias, getInscripcionesCampania, type InscripcionCampania } from "@/lib/usuarioCampaniasApi";

export default function InscripcionesCampaniaUsuarioPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([campanias, ins]) => {
        const c = campanias.find((x) => x.id === numId);
        if (c) setNombreCampania(c.nombre);
